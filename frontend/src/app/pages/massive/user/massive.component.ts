...
export class UserMassiveComponent implements OnInit {
  ...
  //     data:"/assets/templates/PlantillaUsuario.xlsx",
  //   }
  // ]
    ...
    if (file) {
        ...
        this.service.uploadMassiveUsers(formData).subscribe(response => {
          ...
          if (response['status']){
            ...
            setTimeout(()=>this.source.refresh(), 3000);
          } else {
            toast_type = "danger";
            title = "Error";
  ...
  checkErrors(event){
    this.dialog.open(ErrorComponent, {context:{data:{id: event.data['id'], 'type': 'users'}}});
  }
  ...
  DownloadSheets() {
    const url = '/assets/templates/PlantillaUsuario.xlsx';
    const anchor = document.createElement('a');
    anchor.href = url;
    anchor.download = 'PlantillaUsuario.xlsx';
    anchor.click();
  }
